...
async function monitorETHTransactions(wallet_address, acct_number, bank_name, bank_code, receiver_name, db, transac_id, timers, crypto_sent, receiver_amount, current_rate, asset_price) {
  const url = `${process.env.BASE_URL}?module=account&action=txlist&address=${wallet_address}&startblock=0&endblock=99999999&sort=desc&apikey=${process.env.ETHERSCAN_API_KEY}`;
  ...
  return await addToQueue(async () => {
    try {
      ...
      // Filter only incoming ETH transactions (value > 0, to === wallet_address, no token transfers)
      const incomingEthTransactions = transactions.filter(tx => 
        tx.value > 0 && 
        tx.isError === '0' && 
        tx.to.toLowerCase() === wallet_address.toLowerCase()  // Ensure it's an incoming transaction
      );
      console.log(`current_rate:`,current_rate)
      console.log(`asset_price:`, asset_price)

      if (incomingEthTransactions.length === 0) {
        console.log("No incoming ETH transactions found for this address." +" "+ wallet_address);
        return;
      }


        timers[transac_id]['lastTransaction'] = incomingEthTransactions[0];
      ...
      console.log(timers[transac_id]['transactionDate'])
      ...
      if (timers[transac_id]['transactionDate'] >= timers[transac_id]['currentTime'] && timers[transac_id]['transactionDate'] <= timers[transac_id]['futureTime']) {
        ...
        const cryptosent = crypto_sent.trim()
        const expectedAmount = cryptosent.replace(/[^0-9.]/g, "");

        if (actualAmount == expectedAmount) {
             const receiverAmount  = receiver_amount.trim()
          const nairaAmount = receiverAmount.replace(/[^0-9.]/g, "");
...
function monitoringTimer(wallet_address, acct_number, bank_name, bank_code, receiver_name, db, transac_id, timers, crypto_sent, receiver_amount, current_rate,asset_price) {
    timers[transac_id]['monitoringTimer'] = setInterval(() => {
       monitorETHTransactions(wallet_address, acct_number, bank_name, bank_code, receiver_name, db, transac_id, timers, crypto_sent, receiver_amount, current_rate,asset_price)
...
function handleSmallAmount(actualAmount, expectedAmount, current_rate, transac_id, acct_number, bank_name, bank_code, receiver_name, db, wallet_address, timers, asset_price){
  ...
  const currentRate = current_rate.trim()
  const rate = currentRate.replace(/[^0-9.]/g, "");
  const assetprice = asset_price.trim()
const assetPrice = assetprice.replace(/[^0-9.]/g, "");
...
function setETHWalletFlag(wallet_address,db) {
